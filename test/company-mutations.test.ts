...
import { runCreateCompany, runCreateCompanyReturning, runUpdateCompany } from "@/lib/company-mutations";
...
describe("runCreateCompanyReturning", () => {
  it("creates a company and returns its id and name", async () => {
    const db = await createTestDb();
    const result = await runCreateCompanyReturning(db, formOf({ name: "Astilleros Sur" }));
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.company.name).toBe("Astilleros Sur");
  ...
  it("returns a validation error when name is missing", async () => {
    const db = await createTestDb();
    const result = await runCreateCompanyReturning(db, formOf({}));
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error).toBe("El nombre es obligatorio");
  });
  ...
  it("returns a friendly error when the insert throws", async () => {
    ...
    const result = await runCreateCompanyReturning(throwingDb, formOf({ name: "X" }));
    expect(result).toEqual({ ok: false, error: "No se pudo crear la empresa" });
  });
});
